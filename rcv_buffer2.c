...
//gestione del buffer di ricezione che ha una dimensione diversa da quella della finestra che scorre su di esso
//WINDOW è la finestra che va da rcv_base a end_wind, BUFFER è la dimensione fisica del buffer
//il funzionamento praticamente è identico a quello in cui BUFFER e WINDOW sono la stessa cosa, bisogna solo diversificare i campi N e W: il primo usato
//per calcolare le posizioni dei pacchetti e degli zeri, il secondo per aggiornare end_window.
        ...
        if(end_wind > recv_base){
            ...
            if( n_seq == recv_base){
                ...
                while(recvBuffer[i].flag == 1){
                    ...
                    end_wind= (recv_base + W -1) % seq_range_len;
